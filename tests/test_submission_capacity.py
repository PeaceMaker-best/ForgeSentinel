...
from forgesentinel.workspace import WorkspaceError
...
class SubmissionCapacityTests(unittest.TestCase):
    ...
    def submit(
        ...
        with (
            patch.dict(os.environ, {"FORGESENTINEL_ENABLE_SUBMIT": "1"}),
            patch("forgesentinel.pipeline.resolve_token", return_value="token"),
            patch("forgesentinel.pipeline.GitHubClient", return_value=client),
        ):
    ...
    def test_delayed_updated_pull_head_converges_without_repeating_push(self) -> None:
        ...
        with patch("forgesentinel.pipeline.time.sleep") as sleep:
            result = self.submit(pipeline, client)
    ...
    def test_delayed_updated_pull_timeout_preserves_push_and_retry_is_read_only(
        ...
        with (
            patch("forgesentinel.pipeline.time.sleep") as sleep,
    ...
    def test_updated_pull_lost_lease_during_wait_stops_reads(self) -> None:
        ...
        with (
            patch("forgesentinel.pipeline.time.sleep", side_effect=lose_lease),
            self.assertRaisesRegex(StoreError, "lease lost"),
        ):
            self.submit(pipeline, client)
        client.pull_request.assert_called_once()
