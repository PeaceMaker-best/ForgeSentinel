# ForgeSentinel contributor guidance

## Scope

ForgeSentinel discovers public GitHub issues, prepares fixes in disposable clones, and
...
- Scope each Issue and PR around one concrete, independently verifiable capability.
  ForgeSentinel does not use changed-line count to force one capability into artificial
...
- Treat remote branch deletion as a separate terminal cleanup. Delete only an exact
  ForgeSentinel-managed same-repository head after its PR merged, the SHA is unchanged,
