...
def installation_info() -> dict:
    ...
    try:
        distribution = metadata.distribution("forgesentinel")
        result.update(
...
def local_diagnostics(
    ...
    if config is None:
        ...
        report["next_actions"].append(
            "Create missing configuration with forgesentinel init, or correct the selected TOML file."
        )
        return report, False
    ...
    report["databases"] = {
        "tasks": inspect_database(
            config.state_dir / "forgesentinel.sqlite3",
