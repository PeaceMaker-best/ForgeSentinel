...
(() => {
  ...
  const storageKey = "forgesentinel-local-session";
  let token = "";
  ...
  async function today(host) {
    ...
    if (!projects.length) host.append(empty("从一个项目开始", "在终端关联项目并配置仓库策略后，它会出现在这里。"), command("forgesentinel project link /absolute/path/to/project"));
    ...
    host.append(details("本页来源与覆盖信息", result), node("p", "meta", "“重新读取”只读取本地数据。更新 GitHub 缓存请在终端执行："), command("forgesentinel overview refresh"));
  }
  ...
  async function projects(host, projectId, bindingId) {
    ...
    async function load() {
      ...
      try {
        ...
        detail.append(details("入口、关联模块与覆盖说明", guide), section("在目标项目继续"), clientSelect, preview, node("p", "meta", "复制的是 POSIX 终端命令，保留当前 ForgeSentinel 配置来源。它预览客户端配置；实际会话由你使用的 Agent 提供。"));
  ...
  async function settings(host) {
    ...
    host.append(command("forgesentinel doctor --local"), node("p", "meta", "配置发生变化后需重启工作台。浏览器会话最长有效 12 小时，进程退出后立即失效。"));
  }
  ...
  async function render() {
    ...
    document.title = labels[view] + " · ForgeSentinel";
    ...
    if (!token) {
      content.append(hero("连接你的本地工作空间。", "请使用终端打印的完整会话链接打开。链接只在本机、当前进程内有效。"), command("forgesentinel web"));
