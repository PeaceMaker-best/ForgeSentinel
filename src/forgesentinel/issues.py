...
PROPOSAL_MARKER = re.compile(
    r"^<!-- forgesentinel-proposal:v1 repository=(?P<repository>[A-Za-z0-9_.-]+/"
...
def attach_proposal_marker(body: str, *, repository: str, draft_id: str) -> str:
    marker = (
        f"<!-- forgesentinel-proposal:v1 repository={repository.lower()} "
        f"draft_id={draft_id} -->"
    )
    return f"{marker}\n{body.lstrip()}"
...
def proposal_body(body: str, *, repository: str) -> str:
    """Strip and validate optional ForgeSentinel routing metadata."""
