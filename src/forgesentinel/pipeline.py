...
class Pipeline:
    def __init__(self, config: AppConfig, *, harness: Harness | None = None) -> None:
        self.config = config
        database = config.state_dir / "forgesentinel.sqlite3"
    ...
    def _mutation_lease(self, repository: str, issue_number: int) -> Iterator[RunLease]:
        ...
        thread = threading.Thread(
            target=heartbeat,
            name=f"forgesentinel-lease-{issue_number}",
            daemon=True,
        )
        thread.start()
        try:
    ...
    def _save_branch_cleanup_pending_checkpoint(
        ...
        *,
        ...
        try:
            ...
            previous = context.get("checkpoint")
            previous = previous if isinstance(previous, dict) else {}
            next_action = f"forgesentinel branch-cleanup plan {repository}"
            remaining = [
                str(value)
    ...
    def apply_branch_cleanup(
        ...
        *,
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_BRANCH_CLEANUP") != "1":
            raise PolicyError(
                "branch cleanup is disabled; set "
                "FORGESENTINEL_ENABLE_BRANCH_CLEANUP=1 for this command"
            )
    ...
    def attest_portfolio_dependency(
        ...
        *,
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_DEPENDENCY_ATTESTATION") != "1":
            raise PolicyError(
                "dependency attestation is disabled; set "
                "FORGESENTINEL_ENABLE_DEPENDENCY_ATTESTATION=1 for this command"
            )
        actor = reviewed_by.strip()
    ...
    def batch_apply(
        ...
        *,
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_BATCH_APPLY") != "1":
            raise PolicyError(
                "batch apply is disabled; set FORGESENTINEL_ENABLE_BATCH_APPLY=1"
            )
        actor = reviewed_by.strip()
        if actor.casefold() != self.config.github.login.casefold():
            raise PolicyError("--reviewed-by must match the configured GitHub login")
    ...
    def batch_advance(
        ...
        *,
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_BATCH_APPLY") != "1":
            raise PolicyError(
                "batch execution is disabled; set FORGESENTINEL_ENABLE_BATCH_APPLY=1"
            )
    ...
    def create_issue_draft(
        ...
        *,
        ...
        return {
            **draft,
            "next_actions": [
                f"forgesentinel issue duplicate-check {draft['id']}",
                f"forgesentinel issue inspect {draft['id']}",
                (
                    f"FORGESENTINEL_ENABLE_ISSUE_STAGE=1 forgesentinel issue stage "
    ...
    def stage_issue_proposal(
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_ISSUE_STAGE") != "1":
            raise PolicyError(
                "online proposal staging is disabled; set "
                "FORGESENTINEL_ENABLE_ISSUE_STAGE=1 for this command"
            )
        draft = self.issue_draft(draft_id)
        ...
        proposal = client.add_project_issue_proposal(
            ...
            body=staged_body,
            client_mutation_id=f"forgesentinel-stage-{draft_id}",
        )
        ...
        return {
            **self._proposal_summary(proposal),
            "repository": draft["repository"],
            "next_action": (
                f"forgesentinel issue review {proposal.item_id} "
    ...
    def promote_issue_proposal(
        ...
        *,
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_ISSUE_PROMOTION") != "1":
            raise PolicyError(
                "issue promotion is disabled; set "
                "FORGESENTINEL_ENABLE_ISSUE_PROMOTION=1 for this command"
            )
    ...
    def import_context(self, source: Path) -> dict[str, Any]:
        ...
        return {
            **imported,
            ...
            "next_action": (
                f"forgesentinel prepare {repository} {bundle['work_item']['external_id']}"
            ),
            "public_write": False,
        }
    ...
    def storage_gc(
        ...
        if not apply:
            return result
        if os.environ.get("FORGESENTINEL_ENABLE_GC") != "1":
            raise PolicyError(
                "storage GC apply is disabled; set FORGESENTINEL_ENABLE_GC=1"
            )
    ...
    def apply_queue(
        ...
        *,
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_QUEUE_APPLY") != "1":
            raise PolicyError(
                "queue apply is disabled; set FORGESENTINEL_ENABLE_QUEUE_APPLY=1"
            )
        if not 1 <= limit <= 100:
        ...
        for _index in range(limit):
            ...
            thread = threading.Thread(
                ...
                name=f"forgesentinel-queue-{task['id']}",
                daemon=True,
            )
            thread.start()
            try:
    ...
    def cancel_task(
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_QUEUE_APPLY") != "1":
            raise PolicyError(
                "queue mutation is disabled; set FORGESENTINEL_ENABLE_QUEUE_APPLY=1"
            )
        return {
    ...
    def attest_owner_review(self, run_id: str, *, reviewed_by: str) -> dict[str, Any]:
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_OWNER_ATTESTATION") != "1":
            raise PolicyError(
                "owner attestation is disabled; set "
                "FORGESENTINEL_ENABLE_OWNER_ATTESTATION=1 for this command"
            )
        (
            run,
            details,
        ...
        *,
        ...
        def cleanup_handoff() -> dict[str, Any]:
            ...
            handoff = {
                ...
                "next_action": (
                    f"forgesentinel branch-cleanup plan {repository}"
                    if pending
                    else "none"
                ),
            }
        ...
        if os.environ.get("FORGESENTINEL_ENABLE_MERGE") != "1":
            block(
                "merge execution is disabled; set FORGESENTINEL_ENABLE_MERGE=1 "
                "for this command"
            )
    ...
    def _submit_leased(
        ...
        *,
        ...
        policy = self.policy(repository)
        submit_enabled = os.environ.get("FORGESENTINEL_ENABLE_SUBMIT") == "1"
        legacy_submit_enabled = os.environ.get("STARFIX_ENABLE_SUBMIT") == "1"
        if not submit_enabled and not legacy_submit_enabled:
            raise PolicyError(
                "submission is disabled; set FORGESENTINEL_ENABLE_SUBMIT=1 "
                "for this command"
            )
        if reviewed_by.casefold() != self.config.github.login.casefold():
            raise PolicyError(
