...
CONFIG_VERSION = 1
PROJECT_CONFIG_NAMES = (".forgesentinel.toml", "forgesentinel.toml", "starfix.toml")
DIAGNOSTIC_SETTINGS = (
    "project.state_dir",
...
@dataclass(frozen=True, slots=True)
class RunnerConfig:
    image: str = "forgesentinel-runner:latest"
    timeout_seconds: int = 1800
    cpus: float = 4.0
    memory: str = "8g"
    pids_limit: int = 1024
...
def default_user_config_path() -> Path:
    ...
    return (root / "forgesentinel" / "config.toml").resolve()


def default_state_dir() -> Path:
    """Return the per-user runtime-state root without creating it."""
    if os.name == "nt" and os.environ.get("LOCALAPPDATA", "").strip():
        return (Path(os.environ["LOCALAPPDATA"]).expanduser() / "ForgeSentinel").resolve()
    base = os.environ.get("XDG_STATE_HOME", "").strip()
    root = Path(base).expanduser() if base else Path.home() / ".local" / "state"
    return (root / "forgesentinel").resolve()


def default_workspace_dir() -> Path:
    """Return the per-user disposable-workspace root without creating it."""
    if os.name == "nt" and os.environ.get("LOCALAPPDATA", "").strip():
        root = Path(os.environ["LOCALAPPDATA"]).expanduser() / "ForgeSentinel"
    else:
        base = os.environ.get("XDG_DATA_HOME", "").strip()
        root = (
            Path(base).expanduser() / "forgesentinel"
            if base
            else Path.home() / ".local" / "share" / "forgesentinel"
        )
    return (root / "workspaces").resolve()


def discover_project_config(start: Path | None = None) -> Path | None:
    """Find the nearest ForgeSentinel project configuration."""
...
def _merge_layers(user: dict[str, Any], project: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(project_runner, dict):
        ...
        merged_runner["image"] = trusted_runner.get(
            "image", "forgesentinel-runner:latest"
        )
...
def load_config(
    ...
    *,
    ...
    if not user_raw and not project_raw:
        ...
        raise ConfigError(
            f"configuration not found: {expected}; run 'forgesentinel init' first"
        )
    ...
    runner = RunnerConfig(
        image=str(runner_raw.get("image", "forgesentinel-runner:latest")),
    ...
    if not github.login:
        raise ConfigError("[github].login is required; run 'forgesentinel init'")
