...
class ProjectOverview:
    ...
    def _external_items(
        ...
        for row in rows[:20]:
            item = {
                ...
                "trace_command": f"forgesentinel trace {project['repository']} {row['issue_number']}",
                "next_command": f"forgesentinel task inspect {row['run_id']} --live",
            }
            try:
    ...
    def _project(
        ...
        for item in items:
            if item["run_id"]:
                item["trace_command"] = (
                    f"forgesentinel trace {project['repository']} {item['issue_number']}"
                )
        with store._connection() as db:
            pending = db.execute(
            ...
            items.append(
                {
                    ...
                    "source_updated_at": row["updated_at"],
                    "next_command": "forgesentinel overview refresh --project-limit 10",
                    ...
                    "items": [
                        {
                            ...
                            "summary": str(exc)[:300],
                            "next_command": "forgesentinel overview refresh",
                        }
                    ],
                    "omitted": 0,
