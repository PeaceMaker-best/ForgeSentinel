...
class DockerVerifier:
    ...
    def _git_paths(worktree: Path) -> tuple[Path, str]:
        ...
        container_git_dir = "/forgesentinel-git"
        if relative.parts:
    ...
    def _verification_sandbox(
        ...
        *,
        ...
        if verification_dir is None:
            temporary = tempfile.TemporaryDirectory(prefix="forgesentinel-verify-")
    ...
    def _run_container(
        ...
        *,
        ...
        container_name = f"forgesentinel-verify-{uuid.uuid4().hex}"
        docker_command = [
            "docker",
            "run",
            "--rm",
            ...
            "-e",
            "HOME=/forgesentinel-env/home",
            "-e",
            "CI=1",
            "-e",
            "XDG_CACHE_HOME=/forgesentinel-env/cache",
            "-e",
            "UV_CACHE_DIR=/forgesentinel-env/uv-cache",
            "-e",
            "UV_TOOL_DIR=/forgesentinel-env/uv-tools",
            "-e",
            "UV_PYTHON_INSTALL_DIR=/forgesentinel-env/uv-python",
            "-e",
            "PIP_CACHE_DIR=/forgesentinel-env/pip-cache",
            "-e",
            "npm_config_cache=/forgesentinel-env/npm-cache",
            "-e",
            "PNPM_HOME=/forgesentinel-env/pnpm-home",
            "-e",
            "GRADLE_USER_HOME=/forgesentinel-env/gradle",
            "-v",
            f"{worktree.resolve()}:/workspace:rw",
            "-v",
            f"{environment_dir.resolve()}:/forgesentinel-env:rw",
            "-w",
            "/workspace",
        ]
        if git_dir is not None:
            docker_command.extend(["-v", f"{git_dir.resolve()}:/forgesentinel-git:ro"])
        ...
        if log_path is not None:
            ...
            if log_truncated:
                ...
                stored_output = (
                    f"[forgesentinel omitted {omitted} earlier characters; "
