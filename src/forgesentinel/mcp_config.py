...
def client_config(config: AppConfig, workspace: Path, *, client: str) -> dict:
    ...
    if client == "codex":
        ...
        fragment = json.dumps(
            {"mcpServers": {"forgesentinel": {"type": "stdio", **entry}}},
            ensure_ascii=False,
            indent=2,
        )
        ...
        fragment = json.dumps(
            {"servers": {"forgesentinel": {"type": "stdio", **entry}}},
            ensure_ascii=False,
            indent=2,
        )
    ...
    return {
        ...
        "note": "This machine-specific preview is for local client settings. Keep it outside Git. Install forgesentinel[mcp] in the indicated Python environment first.",
        "public_write": False,
    }
