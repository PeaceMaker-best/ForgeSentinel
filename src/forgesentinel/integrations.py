...
SHARED_PATH = ".agents/forgesentinel-context.md"
SHARED_TEXT = """# ForgeSentinel task context

At the start of work, and after switching client or resuming a session, run:

```sh
forgesentinel task current . --format markdown
```
...
`forgesentinel task start . --issue NUMBER --reviewed-by LOGIN`.

Before a handoff, run `forgesentinel task inspect RUN_ID --live`. Write a checkpoint
...
```sh
forgesentinel task checkpoint RUN_ID --expected-revision REVISION \\
...
def _atomic_write(path: Path, value: bytes, *, mode: int = 0o600) -> None:
    ...
    descriptor, temporary = tempfile.mkstemp(prefix=".forgesentinel-", dir=path.parent)
    try:
...
class AgentIntegration:
    ...
    def plan(self, path: Path, *, client: str, revert: bool = False) -> dict[str, Any]:
        ...
        if revert and client in state["clients"]:
            ...
            before = _read(root, relative)
            if before is not None and b"<!-- forgesentinel:" in before:
                raise IntegrationConflict(f"unowned integration marker in {relative}")
            fragment = (
                ("\n\n" if before is not None else "")
                + f"<!-- forgesentinel:{client}:begin -->\n{reference}\n<!-- forgesentinel:{client}:end -->\n"
            )
