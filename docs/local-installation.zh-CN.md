# 从独立安装开始使用 ForgeSentinel

日常维护时可以直接打开目标项目。ForgeSentinel 的安装环境、用户配置、台账和目标
工作区分别管理；开发 ForgeSentinel 本身时才需要进入它的源码 checkout。
...
```bash
uv tool install --python 3.12 /absolute/path/forgesentinel-0.1.0-py3-none-any.whl
...
```bash
uv tool install --python 3.12 'forgesentinel[mcp] @ file:///absolute/path/forgesentinel-0.1.0-py3-none-any.whl'
```
...
```bash
forgesentinel doctor --local
forgesentinel doctor --local --expect-state-dir /absolute/path/to/effective/state
```
...
结构化说明；可通过 `forgesentinel init` 配置身份，或修复所选配置文件后重试。
...
既有 `forgesentinel doctor`。

## 在目标项目继续工作

可用 `forgesentinel web` 启动[本地只读工作台](local-workbench.zh-CN.md)，集中查看已关联
项目、代码导览、任务接续与审阅依据。网页随安装提供，不需要源码 checkout 或 Node。

```bash
cd /absolute/path/to/your-project
forgesentinel project link .
forgesentinel project inspect .
forgesentinel understand scan .
forgesentinel understand guide .
```

关联身份不会自动创建仓库维护权限。需要任务与交付时，再用
`forgesentinel repo add owner/repo --mode maintainer` 或 `--mode contributor` 配置对应
角色和仓库策略。用户已有的 clone/worktree 保持原位置。

按[任务接续指南](coding-agent-assistance.zh-CN.md)创建或选择任务，然后在该目标项目
打开平时的 Agent。`forgesentinel mcp config . --client codex|claude-code|copilot-vscode`
