...
ForgeSentinel 是位于 GitHub、Coding Harness 和隔离验证环境之间的本地优先控制面。仓库策略、
...
ForgeSentinel 与 Coding Model、CI 和 GitHub 项目管理配合使用。它负责约束维护流程，不以批量制造
PR 为目标。
...
[定位 RFC #70](https://github.com/PeaceMaker-best/ForgeSentinel/issues/70) 记录模型自治仓库治理的长期方向。
...
`benchmark run` 完全离线运行 ForgeSentinelBench v0。版本化 fixtures 覆盖安全门槛、上下文边界、
...
GitHub 写入使用配置的维护者身份。ForgeSentinel 在本地审计状态中保存模型、策略、证据、意图和结果
来源。
...
ForgeSentinel 当前版本是 0.1。配置、Schema 和公开接口在 1.0 前仍可能调整。内置 Harness 包括
